...
import models_mae_deit_bootstrap
...
def get_args_parser():
    ...
    parser.add_argument('--no_of_bootstrap', default=5, type=int)
...
def main(args):
    ...
    model = models_mae_deit_bootstrap.MaskedAutoencoderDeiT(
        img_size=args.input_size,
        patch_size=args.patch_size,
        norm_pix_loss=args.norm_pix_loss,
    )

    parent_model = copy.deepcopy(model)
    ...
    # loss_fn = models_mae_deit.Bootstraploss()
    loss_fn = None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, parent_model, data_loader_train,
            optimizer, device, epoch, loss_scaler, loss_fn,
            log_writer=log_writer,
            args=args
        )

        if (epoch + 1) % (args.epochs // args.no_of_bootstrap) == 0:
            print("Bootstraping")
            parent_model = copy.deepcopy(model)
            parent_model.eval().requires_grad_(False)
            parent_model.parent_model = True
